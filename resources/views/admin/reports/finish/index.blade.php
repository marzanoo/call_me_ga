...
<div class="container mx-auto p-4">
    ...
    <h5 class="font-semibold text-gray-800 text-2xl mb-4">Laporan Selesai</h5>
    @if (!session('login_via_superapp'))
    <div class="flex border-b mb-4">
        <a href="{{ route('admin.reports.finish.index', ['status' => 'selesai']) }}"
        class="px-4 py-2 text-sm font-semibold
        {{ $status === 'selesai' ? 'border-b-2 border-[#B3282D] text-[#B3282D]' : 'text-gray-500' }}">
            Selesai
        </a>

        <a href="{{ route('admin.reports.finish.index', ['status' => 'ditolak']) }}"
        class="px-4 py-2 text-sm font-semibold
        {{ $status === 'ditolak' ? 'border-b-2 border-[#B3282D] text-[#B3282D]' : 'text-gray-500' }}">
            Ditolak
        </a>
    </div>
            ...
            <tbody>
                @forelse ($reports as $report)
                    ...
                    <tr>
                        ...
                        <td class="border px-4 py-2 text-sm">
                            @if ($statusData?->status === 'Selesai')
                                <a href="{{ route('admin.reports.finish.done.show', $report->id) }}"
                                class="text-blue-600 hover:underline">
                                    Lihat Detail
                                </a>
                            @elseif ($statusData?->status === 'Ditolak')
                                <a href="{{ route('admin.reports.finish.declined.show', $report->id) }}"
                                class="text-blue-600 hover:underline">
                                    Lihat Detail
                                </a>
                            @endif
    ...
    <div class="bg-white p-4 rounded-lg shadow-md text-center mb-4">
        ...
        <p class="flex text-sm text-gray-500 mb-6">Total: {{ $laporanSelesaiCount }}</p>
        <a href="{{ route('admin.reports.finish.done.index') }}" class="btn block w-full bg-[#B3282D] text-white py-2 px-4 rounded-lg shadow-md">Lihat</a>
    </div>
    <div class="bg-white p-4 rounded-lg shadow-md text-center mb-4">
        <h5 class="flex text-lg font-bold text-gray-800 mb-1">
            Laporan Ditolak
        </h5>
        <p class="flex text-sm text-gray-500 mb-6">Total: {{ $laporanDitolakCount }}</p>
        <a href="{{ route('admin.reports.finish.declined.index') }}" class="btn block w-full bg-[#B3282D] text-white py-2 px-4 rounded-lg shadow-md">Lihat</a>
    </div>
    @endif
</div>
@endsection